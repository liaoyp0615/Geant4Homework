...
void B1SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if(debug) {
    ...
    G4cout << " prepoint->  pos(cm): (" <<pointIn.x()<<","<<pointIn.y()<<","<<pointIn.z()
                   <<")  mom(MeV): (" <<p3_pre.x()<<","<<p3_pre.y()<<","<<p3_pre.z() << ")" << G4endl;
    G4cout << " postpoint->  pos(cm): (" <<pointOut.x()<<","<<pointOut.y()<<","<<pointOut.z()
                   <<")  mom(MeV): (" <<p3_post.x()<<","<<p3_post.y()<<","<<p3_post.z() << ")" << G4endl;
    G4cout << G4endl;
  }
  
  G4double x = pointIn.x();
  G4double y = pointIn.y();
  G4double z = pointIn.z();
  G4double e = edep;

  if (volume == fBodyPhyVolume)  fEventAction->AddBody(x,y,z,e,1);
  if (volume == fTumorPhyVolume) fEventAction->AddTumor(x,y,z,e,1);
  if (volume == fHeadPhyVolume) fEventAction->AddHead(x,y,z,e,1);
  if (volume == fBodyPhyVolume || volume == fTumorPhyVolume || volume == fHeadPhyVolume) fEventAction->AddAll(x,y,z,e,1);
}
