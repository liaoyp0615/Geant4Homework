...
void draw(){
    ...
    TString name = "neutron_gaus";
	TString filename = "root/"+name+".root";
    t->Add( filename );
    ...
    // // --- define histograms (x,y,z,e) for gamma
    // double AllHist[12] = { -15,10,50,  -7,7,50,  -30,30,50,  0,50,50};
    // double TumorHist[12] = { -13,-3,50,  -5,5,50,  -5,5,50,  0,50,50};
    // double BodyHist[12] = { -15,10,50,  -7,7,50,  -30,30,50,  0,50,50};
    // double HeadHist[12] = { -12,12,50,  -12,12,50,  20,60,50,  0,50,50};
    ...
    TCanvas *c1 = new TCanvas("c1","",4000, 3500);
    c1->Divide(4,4);

    c1->cd(1); h_xy->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(A,I)"));
    c1->cd(2); h_xz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(A,II)"));
    c1->cd(3); h_yz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(A,III)"));
    c1->cd(4); h_e->Draw("hist"); lt.DrawLatex(0.64, 0.25, Form("(A,IV)"));
    lt.DrawLatex(0.64, 0.72, Form("All"));
    lt.DrawLatex(0.4, 0.55, name);

    c1->cd(5); ht_xy->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(B,I)"));
    c1->cd(6); ht_xz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(B,II)"));
    c1->cd(7); ht_yz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(B,III)"));
    c1->cd(8); ht_e->Draw("hist"); lt.DrawLatex(0.64, 0.25, Form("(B,IV)"));
    lt.DrawLatex(0.64, 0.72, Form("Tumor"));

    c1->cd(9); hb_xy->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(C,I)"));
    c1->cd(10); hb_xz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(C,II)"));
    c1->cd(11); hb_yz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(C,III)"));
    c1->cd(12); hb_e->Draw("hist"); lt.DrawLatex(0.64, 0.25, Form("(C,IV)"));
    lt.DrawLatex(0.64, 0.72, Form("Body"));

    c1->cd(13); hh_xy->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(D,I)"));
    c1->cd(14); hh_xz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(D,II)"));
    c1->cd(15); hh_yz->Draw("COLZ"); lt.DrawLatex(0.64, 0.25, Form("(D,III)"));
    c1->cd(16); hh_e->Draw("hist"); lt.DrawLatex(0.64, 0.25, Form("(D,IV)"));
    lt.DrawLatex(0.44, 0.72, Form("Head+Neck"));

    c1->Print("figure/"+name+".png");
    c1->Print("figure/"+name+".eps");
